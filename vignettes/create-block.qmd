...
Many options exsist to build expressions, such as using tools offered by the rlang package or by passing a string to `base::parse()`.
As a block developer you are free to choose the tools that your prefer, so long as the evaluated expression returns non-reactive R code which can be executed outside of blockr.
