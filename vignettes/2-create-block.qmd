...
As inputs, a server function takes an id and any additional inputs from other blocks (e.g., data): zero for a __data__ blocks, one for a __transform__ blocks such as a select block, two for a __join__ block and the special argument `...args` for __variadic__ blocks, such as an `rbind` block.
...
Many other options exsist, such as using tools offered by the rlang package or by passing a string to `base::parse()`, e.g.
